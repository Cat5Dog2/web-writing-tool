...
ゲストログインは `POST /login/guest`（フォーム、任意の `returnUrl`）で提供する。匿名で利用でき、CSRFトークン必須、ログイン用レート制限を適用する。成功は `302` で安全なローカルURL（不正・未指定時は `/`）へ遷移する。CSRF不正・フォーム以外は `400`、レート超過は `429`。Identityの非永続Cookieで最大8時間ログインし、ゲストのAPI操作・バックグラウンドジョブは常にモックを使う。ログアウトは既存の `POST /logout` を利用する。
...
レート制限による拒否は、`title: RateLimited`のProblemDetails本文を持つ`429`として返す。
...
認可: 認証必須。CSRFトークン必須。レート制限（`PasswordChange`: ユーザー単位で10分5回）。
...
| `newPassword` | string | 必須 | Identityのパスワードポリシーを満たす新しいパスワード |
| `confirmNewPassword` | string | 必須 | 新しいパスワードの確認入力 |

処理:

- ログインユーザー本人を対象にする。
- 現在パスワードをIdentityで検証する。
- 新しいパスワードと確認入力の一致、現在パスワードとの差異、Identityのパスワードポリシーを検証する。
- 成功後はSecurityStampを更新し、現在の認証Cookieを更新してログイン状態を維持する。
- 現在パスワード、新しいパスワード、確認入力はログ、監査ログ、レスポンスへ出さない。
...
| --- | --- |
| `204 No Content` | 変更成功 |
| `400 Bad Request` | 現在パスワード不一致、確認不一致、現在と同じパスワード、パスワードポリシー違反 |
| `404 Not Found` | ログインユーザーが存在しない |
| `429 Too Many Requests` | レート制限超過 |
| `500 Internal Server Error` | Identityの非バリデーションエラーで変更できない |
...
- 対象ユーザーの存在を確認する。
- MVPでは管理者によるメールアドレス変更とパスワード再設定は扱わない。ユーザー本人のパスワード変更は`PUT /api/account/password`で扱う。
