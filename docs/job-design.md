...
`ExternalApiDeferredException`は送信前に判明した利用枠不足を表す。`Running`のジョブを`Queued`へ戻し、`NextRunAt`に枠の再開予定を保存する。この取得で加算した`AttemptCount`を戻し、`Progress`と`StartedAt`を保持してロックを解除する。同じ待機処理が再度届いた場合は、既に`Queued`のジョブを変更しない。待機自体で最大試行回数に達したり失敗通知を送信したりしない。停止予約がある一括生成は従来どおり停止へ遷移する。

次回の取得では`StartedAt`が存在するジョブを`IsResuming`としてHandlerへ渡す。本文生成は再試行回数が1のままでも保存済みの完成本文を飛ばし、未完了の見出しだけを生成する。待機のための失敗生成ログ・文字数会計は追加しない。実際に送信した結果の429は従来の最大試行回数に従い、Gemini Clientが共有した待機時間以降に再試行する。
...
通常ユーザーのジョブは初期実装では期間による削除をしない。期限切れゲストのジョブは関連データとともに自動削除するが、Runningジョブがあるユーザーは完了後に回収する。`GuestAccountCleanupWorker` は起動時と1分ごとに実行し、ゲスト単位でDBトランザクションを使う。処理待ちジョブの取得と削除判定は行ロックで直列化する。運用後に以下を検討する。
