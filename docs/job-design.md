...
- Worker専用コンテナをMVPから分離するか。
- ジョブ種別ごとの並列度を初期実装で持つか。
- SignalRでジョブ状態をプッシュ通知するか。
- AI Providerごとの詳細なトークン/文字数換算ルール。
- 失敗ジョブの管理者画面をMVPに含めるか。
