...
- 本人退会時は対象ユーザーに紐づく記事、見出し、ジョブ、検索結果、X投稿キャッシュ、WordPress連携、通知設定、利用履歴、対象ユーザーが操作ユーザーの監査ログなどの業務データを物理削除する。
- 本人退会時に`Running`ジョブがある場合は退会を拒否する。
- 最後のAdminユーザーは本人退会できない。
- 管理者は他のユーザーを削除できる。
- 最後のAdminユーザーの降格、無効化、削除は拒否する。
- 管理者がユーザーを削除する場合、そのユーザーに紐づく記事、見出し、ジョブ、検索結果、X投稿キャッシュ、WordPress連携、通知設定、利用履歴、対象ユーザーが操作ユーザーの監査ログなどの業務データも物理削除する。
...
- 記事は下書き、構成作成中、構成作成済み、本文生成中、完了、投稿済み、失敗のステータスを持つ。
...
| Article | ID、UserId、Keyword、Title、Status、Tone、Tags、Memo、Body、HtmlBody、MetaDescription、WritingProfileWordpressSiteId、WritingProfileSnapshotJson、AutoPostToWordpress、AutoPostWordpressSiteId、DeletedAt |
...
| WordpressSite | ID、UserId、SiteName、BaseUrl、LoginId、EncryptedAppPassword、DefaultCategoryId、SiteAdminProfile、WritingCharacter、ReaderPersona |
...
- ユーザー間のデータ分離を徹底する。
- CSRF対策を行う。
- 外部APIキー、WordPress Application Password、Discord Webhook URLは暗号化または安全なシークレット管理に保存する。
