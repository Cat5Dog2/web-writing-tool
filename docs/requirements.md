...
- 外部APIを利用せず、タイトルの作成、Web/X検索のサンプル取得・表示、その参考情報を使った構成・本文の作成から保存まで動作確認できるダミーモードを提供する。サンプルは通常モードの検索キャッシュと分離する。詳細は [設定リファレンス](configuration-reference.md#612-ダミーモード) を参照。
...
ゲストログインでは、登録不要の専用一般ユーザーで記事作成を体験できる。ゲストごとにデータを分離し、生成・検索は常にモックを使う。Gemini、Tavily、X、WordPress、Discordの実APIには接続しない。通常ユーザーには既存の外部API設定を適用する。ゲストのアカウントと関連データは作成から8時間経過後に自動削除する。実行中ジョブがある場合は完了後に削除する。
...
- 主要ナビゲーションは以下を想定する。
  ...
  - 画像生成
  - リライト
- MVPの通常記事管理画面では、文字数上限設定、モデル名、残り構成数などの利用上限カウントは表示しない。利用上限の管理は管理者機能で扱う。
...
- ユーザーは現在パスワードを再確認し、Identityのパスワードポリシーを満たす新しいパスワードへ変更できる。
...
- MVPで採用するAIモデルはGoogle Gemini 3.8 Flashとする。
- テキスト生成モデルは将来複数選択できる設計とし、MVPの既定モデルはGoogle Gemini 3.8 Flashとする。Google Gemini 3.7 Flash、Google Gemini 3.6 Flash、Google Gemini 3.5 Flashも選択可能なモデルとして残す。
...
- Gemini 3.8 FlashのAPIモデルIDは`gemini-3.8-flash`、Gemini 3.7 FlashのAPIモデルIDは`gemini-3.7-flash`、Gemini 3.6 FlashのAPIモデルIDは`gemini-3.6-flash`、Gemini 3.5 FlashのAPIモデルIDは`gemini-3.5-flash`とする。
- Gemini 3.8 Flash、Gemini 3.7 Flash、Gemini 3.6 Flash、Gemini 3.5 Flashの利用可能リージョンはJapanとする。
