...
- 以下の一括設定を指定できる。
  - 生成範囲（記事完成までを既定とし、構成のみも選択可能）
  - H2の個数
  - H3の個数
  - 情報を日本国内に限定する
  - 作成対象モデル
  - Web検索とX検索の個別有効/無効、各検索件数、X検索期間
  - サイト別ライティング設定
  - WordPress自動投稿の有効/無効
  - 自動投稿先WordPressサイト
  - 自動投稿カテゴリ
- 登録実行後、記事作成ジョブを複数作成する。
- 未入力タイトルは自動生成し、入力済みタイトルは維持する。記事完成までの場合は構成・メタディスクリプション・全見出し本文・HTMLを保存する。
- 一覧・詳細に記事別の処理段階と本文進捗を表示し、停止・未完了部分からの再開を提供する。
- 完了した一括生成の進捗は履歴として折りたたみ、処理中・失敗・停止中の記事を優先表示する。
- 記事編集では本文を優先配置し、モバイルは保存・保存してプレビューを下部に固定する。保存してプレビューは保存と表示更新を一度に実行し、失敗時は入力を保持する。保存済み本文の再生成は対象を示して確認する。画面操作の詳細は`screen-design.md`に従う。
- モバイル編集では保存状態とエラー確認を固定保存欄に表示する。前後の見出し移動にも未保存確認を適用し、削除は対象・配下の範囲をダイアログで確認する。横向きなど高さの小さい画面は集中表示を既定とし、通常表示へ戻せる。ソフトウェアキーボードによる可視領域の縮小に追従し、端末の拡大表示は妨げない。
- 登録履歴を切り替えても、他の登録の処理中・失敗・手動停止記事を追跡できる。入力エラーは該当項目を示し、スマートフォンでもその場で修正できる。
...
- 選択した検索を記事単位で実行して各生成段階で共有する。無効な検索ソースは保存済み資料も生成に使用しない。検索エラーは該当記事で停止し、結果0件は警告を表示して続行する。
- 生成段階の完了と次のジョブの登録は同じトランザクションで保存する。再実行時は保存済み本文を維持し、未完了部分だけを生成する。
- ゲストは通常ユーザーと同じ処理を進め、外部APIの代わりにサンプルを使用する。従来APIで生成設定を省略した場合は構成生成までの互換動作を維持する。
...
- 編集中の未保存変更は見出し・画面切替や生成操作で黙って破棄されず、保存・破棄・キャンセルを選べる。
- 構成作成ボタンから1回の操作で構成ジョブが登録され、結果を編集画面で確認できる。
- ゲストが利用できない外部連携は各画面でも明示し、認証情報の入力を促さない。
- スマートフォンの縦・横向きで本文・見出し・操作ボタンへ到達でき、モーダルはキーボードで開閉できる。
- UIの詳細な受け入れ条件は[画面設計27章](screen-design.md#27-uiuxレビュー反映t-1333t-1338)に従う。
