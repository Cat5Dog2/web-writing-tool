...
- 編集画面の「保存してプレビュー」は、変更のある記事情報と見出し本文を保存してから既存のHTML変換処理を実行する。保存・変換に失敗した場合は画面遷移せず、入力を保持する。変換成功後に保存済みHTMLをプレビューする。
- 見出しの変更により保存HTMLが無効化された場合は、プレビューに更新手順を表示する。目次のH2/H3は入れ子のリストで表示し、保存HTMLのサニタイズとローカルID生成は維持する。
...
| 投稿ステータス | 未指定時は`Draft` |
| `Publish` | `HumanReviewRequired = true`かつ`HumanReviewedAt`未設定では拒否 |
...
| 人間確認未完了でPublish指定 | `HumanReviewRequired` | `HumanReviewedAt`未設定の公開投稿を拒否する |
