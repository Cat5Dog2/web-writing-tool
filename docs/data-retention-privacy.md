...
- 検索キャッシュの通常/ダミー区分は `IsDummy` で分離する。サンプルにも同じ保持期限を適用し、通常のX再取得でサンプルや他ユーザーの行を更新しない。生成へ渡す内容は各資料500文字までの抜粋と取得日時を基本とし、プロンプト全文のログ保存は行わない。
...
一括自動生成の`ArticleGenerationRuns`も対象記事の物理削除に伴い削除する。設定・進捗履歴をゲスト削除後に残さない。実行中ジョブがある間の削除延期は従来どおり適用する。

ゲストごとにIdentityユーザーとゲストClaimを保存し、記事・ジョブ・検索結果・利用履歴を通常と同じDBへ保存する。メールアドレスとパスワードは収集しない。保存データはユーザーIDで分離する。

ゲストアカウントの `CreatedAt + 8時間` をCookieの有効期限とDBデータの保持期限に共通で使用する。操作・ログアウト・再送で延長しない。ゲスト自身はログアウト後に同じユーザーへ再ログインできず、次のゲストログインでは別ユーザーになる。

`GuestAccountCleanupWorker` が起動時と1分ごとに、期限を迎えたゲストClaim付きユーザーを古い順に最大100件回収する。稼働DB上のIdentity情報、記事・見出し（論理削除済みを含む）、ジョブ、検索結果、設定、利用・生成・通知・投稿履歴、本人の既存監査ログをユーザー単位のトランザクションで物理削除する。削除途中の失敗はロールバックし、次回再試行する。既存バックアップは5.3の保持期限に従う。

Runningジョブがあるユーザーは完了後の巡回で削除する。ユーザー行とジョブ行のロックで新規データの挿入・ジョブ取得との競合を制御し、削除直前にも期限・ゲストClaim・Adminロール・Running状態を確認する。通常ユーザーとAdminへ昇格済みのユーザーは自動削除しない。

アプリ停止中または `BackgroundJobs:Enabled=false` の間は自動削除しない。再起動・再有効化後の巡回で回収する。停止・実行中ジョブ・削除待ち件数によって8時間経過から削除まで遅れることがある。
...
本人退会、管理者によるユーザー削除、期限切れゲストの自動削除では、対象ユーザーに紐づく業務データをトランザクション内で物理削除する。
...
期限切れ検索キャッシュ削除は専用の`SearchCacheCleanupWorker`が定期実行し、以下を行う。ユーザー所有の`ArticleGenerationJobs`には登録しない。
...
- 期限切れ検索キャッシュ削除Workerが件数のみログ出力する。
