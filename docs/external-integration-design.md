...
`ExternalApis:UseMocks=true` の場合は各インターフェースをダミーClientへ切り替える。生成・Web検索・X検索はサンプルを返す。サンプルX投稿は実APIで再取得せず維持する。WordPress投稿・Discord通知は送信せず失敗結果を返す。実API Clientへはフォールバックしない。設定・制約は [設定リファレンス](configuration-reference.md#612-ダミーモード) を参照。
...
- Gemini Clientが`Temperature`を破棄した場合は`LogDebug`に記録し、設定が黙って無効化されたように見えないようにする。
- 出力のばらつきを制御する場合は`SystemInstruction`に明示ルールを書く。
...
- X投稿は所有者・記事・見出し・通常/ダミー区分・検索条件・`PostId`の組で一意とし、同じ投稿を別の記事で取得できる。期限切れの同じ組を再検索した場合は内容・TTLを更新する。
- キャッシュが有効な場合は外部APIを呼ばず、DB上の検索結果を利用する。
- Web検索は手動取得を `IsManual=true`、生成内の自動取得をfalseで保存する。手動操作がキャッシュに一致した場合も手動優先へ更新し、自動検索では優先指定を解除しない。表示・生成とも手動資料を先に並べ、同じ優先度では取得日時・検索順位の順に並べる。生成へのWeb採用上限10件は維持し、残りの枠に自動結果を入れる。
- 記事編集画面の検索操作は専用ジョブを登録する。Web検索有効時の構成・本文生成、およびRefreshは生成ジョブ内でWeb検索を完了させてから参考情報をAIへ渡す。X検索は明示操作時のみで、既に取得した有効な投稿を利用する。表示と生成への利用前には既存の再取得ポリシーを適用し、失敗した投稿は利用しない。
...
一括登録の自動生成ではWeb・X検索を独立して指定できる。各検索は記事単位で行い、無効なソースは保存済み資料も生成に渡さない。検索件数・X検索期間は登録時に保存し、X期間の基準時刻は登録日時に固定する。資料の有効期限・通常/ダミー分離・X再取得のルールは維持する。選択されたX資料の再取得に失敗した場合は、その生成段階を失敗させて再試行する。
