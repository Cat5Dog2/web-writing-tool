...
| Options | 主な項目 |
| --- | --- |
| `AiProviderOptions` | Provider、Model、ApiKey、Timeout、MaxInputChars。MVPの既定はGoogle Gemini 3.6 Flash |
...
MVPの採用ProviderはGoogle Gemini、既定モデルはGoogle Gemini 3.6 Flashとする。APIモデルIDは`gemini-3.6-flash`、利用可能リージョンはJapanとする。Google Gemini 3.5 Flash（`gemini-3.5-flash`）も選択可能なモデルとして残す。
...
- Gemini 3.xでは`temperature`、`top_p`、`top_k`が非推奨であり、公式ガイドは全リクエストからの削除を案内している。`gemini-3.5-flash`も対象に含む。
