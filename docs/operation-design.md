...
- 本番DBバックアップと`app_keys`のバックアップの取得手順を確認する。取得自体は14.2の順序で行う。スキャン成功後、Migration前が正しいタイミングになる。

### 14.2 デプロイ手順

1. 本番VPSへログインする。
2. 対象リリースのソースを配置する。
3. `scripts/scan-image.ps1 -Build`でイメージをビルドし、同じ成果物をスキャンする。DBへ触れる前に行う。ここで失敗したら中断し、DBは変更しないまま残す。
4. `docker compose stop app`でappを停止する。
5. 本番DBバックアップと`app_keys`のバックアップを取得する。
6. 必要に応じて`docker compose --profile tools run --rm migrate`でDBマイグレーションを実行する。
7. `docker compose up -d --no-build`でサービスを更新する。`--build`を付けない。付けるとスキャンを通した成果物ではなく、その場で作り直した別の成果物が動く。
8. `docker compose ps`で起動状態を確認する。
9. 公開URLから`curl -fsS`で`/health/live`を確認する。`/health/ready`はVPS上からループバック経由で確認する。7.2参照。
10. 管理者でログインし、`/health/deps`が`Healthy`であることを確認する。外部APIキーの設定漏れはここでしか検知できない。
11. 記事一覧、記事作成ジョブ登録を確認する。
12. Caddyログとアプリログに異常がないことを確認する。

スキャンをMigrationより前に置くのは、順序を逆にすると失敗時の状態が壊れるためである。
Migrationを先に適用してからスキャンが失敗すると、新しいスキーマだけがDBへ入り、
