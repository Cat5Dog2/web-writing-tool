...
- 外部APIキー、DBパスワード、WordPress Application Passwordは環境変数またはVPS上の秘密情報ファイルで管理する。
...
| 項目 | 確認内容 |
| --- | --- |
| 本番ドメイン | Caddyfile、Cookieドメイン、通知URLに影響 |
| VPSスペック | 同時利用者数、記事生成数により調整 |
| 外部バックアップ先 | S3互換ストレージ、別VPS、クラウドストレージ等 |
| 監視サービス | Uptime監視、メトリクス基盤、通知先 |
| worker分離時期 | ジョブ量、メモリ使用量、レスポンス影響で判断 |
| ログ保存期間 | 契約、プライバシーポリシー、ディスク容量で判断 |
| AI使用量制限 | 月次上限、ユーザー別上限、停止時の通知方法 |
