...
- ダミーモードでも認証・認可と既存の公開前確認を維持する。外部API Clientは実APIへフォールバックせず、WordPress投稿・Discord通知を送信しない。設定の詳細は [設定リファレンス](configuration-reference.md#612-ダミーモード) を参照。
- 参考情報APIと検索サービスで記事・見出しの所有者を確認する。画面は外部本文をテキストとしてエスケープし、出典リンクは認証情報を含まないHTTPSのみ許可する。X再取得失敗時はキャッシュをそのまま表示・生成利用しない。生成AIへ渡す参考情報は未検証データとして明示的に区切る。
...
- `POST /login/guest` は匿名利用を許可するが、CSRF検証と既存のログイン用レート制限を必須にする。戻り先はローカルURLに限定する。
- Identityでメール・パスワードを持たない一意のユーザーを作成する。Userロールと `web-writing-tool:guest=true` Claimを同一DBトランザクションで保存し、Admin権限は付与しない。
- 既存のIdentity Cookieを使用し、永続化なし・延長なし・8時間の有効期限にする。ログイン済みゲストの再送は同じセッションを維持する。通常ユーザーがゲストログインを選んだ場合は新しいゲストセッションに切り替える。
- ゲストClaimはCookie更新時にも維持する。外部API Clientと検索データモードはユーザーのスコープで切り替え、ジョブ実行時はCookieや入力Payloadに頼らず、DB上のジョブ所有者のClaimを読み直してから専用スコープでHandlerを解決する。
- ゲストの検索・生成・再取得はモックに限定し、WordPress投稿・Discord通知を送信しない。既存の所有者認可・CSRF・利用制限を維持する。パスワードを持たないため、現在パスワードの確認が必要な変更・退会処理は成功しない。
- ログアウトは既存のCSRF保護された経路でCookieを破棄する。保存データの扱いは [データ保持・プライバシー](data-retention-privacy.md#54-ゲストデータ) を参照。
- Cookie期限とゲストデータ削除期限は作成日時から8時間に揃える。ゲストClaim・期限・Admin非該当をDBで確認してから関連データを物理削除し、Runningジョブがある間は削除を延期する。通常ユーザーを日時やユーザー名だけで削除しない。
...
Cookie認証のchallenge/forbidでは、`/api`配下に401/403のProblemDetailsを返す。リダイレクトによるHTML応答やステータスコードページへの再実行を防ぎ、APIクライアントが認証エラーを判別できるようにする。画面へのアクセスは既存のログイン・権限不足ページへの遷移を維持する。実装と回帰確認は`T-1346`を参照。
...
- 初期AdminのメールアドレスとパスワードはUser Secrets、ローカル環境変数、Git管理外の`.env`、またはVPS上の秘密情報ファイルから取得する。
...
- 認証済みユーザー本人だけが変更できる。
- 現在パスワードの再確認、確認入力との一致、Identityのパスワードポリシーを必須にする。
- 現在と同じパスワードへの変更を拒否する。
- 変更成功後はSecurityStampと現在の認証Cookieを更新する。
- パスワード値はログ、監査ログ、ProblemDetails、画面クエリへ出さない。
- 初期Adminのパスワード変更後は`AdminSeed__Email`と`AdminSeed__Password`を`.env`から削除または空にする。
...
- 外部APIキー、DBパスワードなどシステム単位の秘密情報は、VPS上の環境変数または権限制限された秘密情報ファイルで管理する。
- Discord Webhook URLはユーザー別にDB暗号化保存し、環境変数による全体共有Webhookは使用しない。
...
Geminiの利用枠はAPIキーを保存・ハッシュ識別せず、設定した非秘密の`Scope`とモデル単位で共有する。429のレスポンスは待機秒数・日時・構造化された日次割当分類だけを読み、プロバイダーのエラー本文をログ・DB・画面へ転送しない。送信予約テーブルには本文・ユーザー・記事の識別情報を含めない。ゲストとダミーモードは実API・実API用の枠予約を行わない。
...
- `Publish`指定時でも`HumanReviewRequired = true`かつ`HumanReviewedAt`未設定の場合は公開投稿を抑止する。
...
- Discord Webhook URLはユーザー別にDB暗号化保存する。
- 環境変数による全体共有Webhookは使用しない。
- 画面表示ではマスクする。
...
- 対象ユーザーが操作した既存監査ログは物理削除し、削除監査ログだけをFKなしのスナップショットで残す。
...
| 種別 | Path | 公開範囲 | 内容 |
| --- | --- | --- | --- |
| Liveness | `/health/live` | 外部監視可 | アプリプロセス稼働 |
| Readiness | `/health/ready` | 内部または管理者限定 | DB接続、BackgroundService状態 |
| Dependency | `/health/deps` | 管理者限定 | Gemini、Tavily、X、WordPress、Discordの疎通状況 |
...
- NuGetパッケージとDockerイメージの脆弱性確認を実行できる。

## 28. セキュリティ強化ロードマップ

| 項目 | MVP方針 | 導入・見直し条件 |
| --- | --- | --- |
| MFA | MVPでは必須化しない。管理者アカウントから後続導入する。 | 複数Admin、有料ユーザー運用、外部公開後の本格運用前 |
| CSP強制化 | MVP初期はReport-Onlyで運用し、違反ログを確認する。 | 違反が解消され、通常操作で問題が出ないことを確認後に強制化 |
| Secret Manager | 単一VPSでは環境変数または権限制限された秘密情報ファイルを使う。 | 複数環境、複数運用者、自動デプロイ、秘密情報の棚卸し頻度増加 |
| 暗号鍵ローテーション | MVPでは手順化して手動実施する。 | 漏えい疑い、運用者変更、定期棚卸し、専用Secret Manager導入時 |
| WAF | MVPでは専用WAFを導入しない。Caddyとアプリ側対策を基本とする。 | 攻撃兆候、Bot増加、公開範囲拡大、管理負荷増加 |
| 脆弱性スキャン | MVPからNuGetパッケージとDockerイメージの確認を行う。 | CI整備時に自動化し、重大脆弱性検出時はリリースを止める |
