...
| `GET` | `/api/articles/{id}` | 記事詳細取得 | 所有者または管理者 |
| `PUT` | `/api/articles/{id}` | 記事更新 | 所有者または管理者 |
| `DELETE` | `/api/articles/{id}` | 論理削除 | 所有者または管理者 |
| `POST` | `/api/articles/{id}/title-candidates` | タイトル候補生成ジョブ登録 | 所有者または管理者 |
| `POST` | `/api/articles/{id}/outline` | 構成生成ジョブ登録 | 所有者または管理者 |
| `POST` | `/api/articles/{id}/headings/{headingId}/body` | 本文生成ジョブ登録 | 所有者または管理者 |
| `POST` | `/api/articles/{id}/wordpress-posts` | WordPress投稿ジョブ登録 | 所有者または管理者 |
| `GET` | `/api/jobs/{id}` | ジョブ状態取得 | 所有者または管理者 |
| `POST` | `/api/wordpress-sites` | WordPressサイト登録 | 認証必須 |
| `POST` | `/api/wordpress-sites/{id}/test` | WordPress接続テスト | 所有者または管理者 |
...
| `PUT` | `/api/admin/users/{id}` | 表示名・有効状態変更 | 管理者 |
| `PUT` | `/api/admin/users/{id}/role` | ユーザーロール変更 | 管理者 |
| `PUT` | `/api/admin/users/{id}/usage-limit` | 利用上限更新 | 管理者 |
| `DELETE` | `/api/admin/users/{id}` | ユーザー物理削除 | 管理者 |
...
- ユーザーは現在パスワードを再確認したうえで本人退会できる。退会時は対象ユーザーに紐づく業務データと、対象ユーザーが操作ユーザーの監査ログをトランザクション内で物理削除する。
