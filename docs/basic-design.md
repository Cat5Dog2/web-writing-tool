...
MVPの主要フローは実装済みである。本書は標準構成、責務分割、データ設計、画面設計、ジョブ処理設計を定め、実装と設計の整合を保つための基準として維持する。
...
| Options | 主な設定 |
| --- | --- |
| `AiProviderOptions` | Provider、Model、API Key、Timeout、MaxInputChars。MVPの既定はGoogle Gemini 3.6 Flash |
...
秘密情報は開発時はUser Secrets、ローカル環境変数、またはGit管理外の`.env`から読み込む。
開発用.NET SDKコンテナでUser Secretsを使う場合は保存先を永続化する。
本番では環境変数またはVPS上の安全なシークレット配置から読み込む。
...
| `IsDomesticOnly` | bool | 必須 | 既定はtrue |
...
| `POST` | `/api/articles/{articleId}/human-review` | 人間確認完了 | 所有者または管理者 |
...
- `HumanReviewRequired = true`の記事は、`HumanReviewedAt`が設定されるまでWordPress公開投稿できない。下書き投稿は可能とする。
- 人間確認後にタイトル、本文、HTML本文、メタディスクリプション、TopicRisk、X引用など公開判断に影響する値を変更した場合、確認済み状態を解除する。
...
| `Memo` | text | nullable |
| `SuggestedKeywords` | text | nullable |
| `RelatedKeywords` | text | nullable |
| `LearningType` | varchar(40) | nullable |
| `LearningText` | text | nullable |
| `AdditionalPrompt` | text | nullable |
| `Body` | text | nullable |
...
MVPではGoogle Geminiを採用し、既定モデルをGoogle Gemini 3.6 Flashとする。APIモデルIDは`gemini-3.6-flash`、利用可能リージョンはJapanとする。Google Gemini 3.5 Flash（`gemini-3.5-flash`）も選択可能なモデルとして残す。
