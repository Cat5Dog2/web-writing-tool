...
| 中 | User Secrets | ローカル開発用の秘密情報。開発用.NET SDKコンテナ利用時は保存先を永続化する |
...
| `ConnectionStrings__DefaultConnection` | `ConnectionStrings:DefaultConnection` | 必須 | 必須 | Yes | PostgreSQL接続文字列。本番Composeでは`POSTGRES_*`からサービス環境変数として生成する |
...
`POSTGRES_*` はPostgreSQLコンテナ初期化用である。アプリとMigrationサービスの`ConnectionStrings__DefaultConnection`はComposeが同じ値から生成するため、`.env.production.example`へ接続文字列を重複定義しない。
...
生成に使うモデルIDは次の優先順位で決まる。

1. ジョブPayloadの`GenerationModel`（再生成、リライトなどで明示指定した場合）
2. 記事の`GenerationModel`（記事作成時に画面で選択した値を保存する）
3. `AiProviders__Gemini__Model`
...
BackgroundJobs__SearchCacheCleanupIntervalMinutes=60
...
Webプロジェクト作成後、開発用.NET SDKコンテナ経由で設定する。
共通入口は`scripts/dotnet.ps1`とし、`~/.microsoft/usersecrets`相当の保存先を永続化する。
