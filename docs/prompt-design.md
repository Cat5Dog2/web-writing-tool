...
対象は、タイトル候補生成、見出し構成生成、本文生成、要約、長文化、リライト、再取得である。MVPのAI ProviderはGoogle Gemini、既定モデルは`gemini-3.7-flash`とする。
...
- MVPの既定モデルは`gemini-3.7-flash`とする。`gemini-3.6-flash`と`gemini-3.5-flash`も選択可能なモデルとして残す。
- プロンプト全文は保存しない。
