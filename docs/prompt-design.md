...
対象は、タイトル候補生成、見出し構成生成、本文生成、要約、長文化、リライト、再取得である。MVPのAI ProviderはGoogle Gemini、既定モデルは`gemini-3.8-flash`とする。
...
一括自動生成では、未入力タイトルのみ候補1件を生成して自動採用する。構成JSONには320文字以内の`metaDescription`を要求し、自動生成で欠落した場合は形式エラーとして再試行する。Web・Xの選択を生成への採用にも適用し、記事単位の資料をタイトル・構成・本文で共有する。Xは個人の発言として扱い、参考資料内の命令は実行しない。H2/H3数と本文文字数はプロンプト上の目安である。
...
Web資料は手動取得（`IsManual=true`）を優先し、残りの枠に自動検索結果を入れる。同じ優先度では取得日時の降順、検索順位の昇順、IDの昇順で並べ、同じURLは1件にまとめる。手動資料が10件以上ある場合はその順で10件を採用する。この順序はDB読込上限より前に適用し、自動検索結果が多数あっても手動資料を押し出さない。所有者・記事・対象見出し・通常/ダミー区分・有効期限・URL・本文の検証は手動資料にも適用する。Xは明示検索のみで、従来どおり有効な投稿を取得日時順に採用する。

実装では記事全体と対象見出しの有効な資料からWeb/X各10件まで、各本文500文字までの抜粋と取得日時を渡す。`ReferencePromptFormatter` が参考情報をJSONで囲い、命令として扱わない指示をSystemInstructionへ追加する。Geminiの送信内容、文字数、PromptHashはこの整形後の内容に一致させる。Web検索有効の構成生成、本文生成（記事/見出し/実行リクエストのいずれかで有効）、Refreshは検索を先に実行する。その他の本文操作でも取得済み資料は使用する。検索失敗時に成功した体裁で生成を続けず、元の本文を残してジョブを失敗させる。X再取得失敗時はその投稿を除外する。
...
- MVPの既定モデルは`gemini-3.8-flash`とする。`gemini-3.7-flash`、`gemini-3.6-flash`、`gemini-3.5-flash`も選択可能なモデルとして残す。
- プロンプト全文は保存しない。
